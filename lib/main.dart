...
const kGoogleApiKey = 'YOUR_API_KEY'; //replace with your api key
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('PlaceZ'),
        actions: <Widget>[
          isLoading ? IconButton(icon: Icon(Icons.timer), onPressed: () {})
    ...
    final placesWidget = places.map((f) {
      List<Widget> list = [
        Padding(
          ...
          child: Text(
            f.name, style: Theme.of(context).textTheme.subtitle,
          ),
        )
      ];
      if (f.formattedAddress != null) {
        list.add(Padding(
          ...
          child: Text(
            f.formattedAddress, style: Theme.of(context).textTheme.subtitle,
          ),
        ));
      }
      if (f.vicinity != null) {
        list.add(Padding(
          padding: EdgeInsets.only(bottom: 2.0),
          child: Text(
            f.vicinity, style: Theme.of(context).textTheme.body1,
          ),
        ));
      }
      if (f.types?.first != null) {
        list.add(Padding(
          ...
          child: Text(
            f.types.first, style: Theme.of(context).textTheme.caption,
          ),
        ));
      }
